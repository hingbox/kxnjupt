...
import logging
class njupt(scrapy.Spider):
    name = "njupt"
    allowed_domain = ["njupt.edu.cn"]
    start_url = ["http://news.njupt.edu.cn/7822/list.htm",]
    def parse(self, response):
        new_page_num = 14
        page_num = 456
        if response.status == 200:
            for i in range(1,page_num+1):
                for j in range(1,new_page_num+1):
                    item = NjuptItem()
                    item["new_url"],item["new_title"],item["new_date"]= response.xpath(
